...
for(var _i=0; _i < _array_size; _i++){
	...
	if _i ==item_index_{
		draw_sprite(s_item_cursor,image_index/8,_box_x,_box_y);
		if instance_exists(_item){
			draw_text(_x+20,_y-100,_item.description_);			
			var _description_height = string_height(_item.description_);
			draw_text(_x+20,_y-20-_description_height, "$ cost: "+string(_item.cost_));
		}
	}
}
